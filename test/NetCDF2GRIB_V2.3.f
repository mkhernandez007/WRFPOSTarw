...
C
        ...
        dlat  = abs((glatmax-glatmin)/latdim)*111.2
        dlon  = abs((glonmax-glonmin)/londim)*111.2
        truelat1 = glatmax - ((glatmax - glatmin)/2.) + 15.  
        truelat2 = glatmin + ((glatmax - glatmin)/2.) - 15.
        cenlon   = glonmax - ((glonmax - glonmin)/2.)
...
C
        ...
        KGDS(6)  = 136      ! Direction increments given, Earth assumed
                            ...
                            ! respectively
        KGDS(7)  = cenlon   ! central longitude that holds true as a y-axis
        KGDS(8)  = dlon     ! longitudinal direction of increment
        KGDS(9)  = dlat     ! latitudinal direction increment
        KGDS(10) = 01       ! projection center flag
        KGDS(11) = 11100000 ! points scanned in +i direction, +j
                            ! direction, Adjacent points in i are
                            ! consecutive, reserved
        KGDS(12) = truelat1 ! Latitudes that hold true
        KGDS(13) = truelat2 ! Latitudes that hold true
